...
const envSchema = z.object({
  ...
  JWT_SECRET: z.string(),
  JWT_EXPIRES_IN: z.number().optional().default(15 * 60),
  REFRESH_EXPIRES_IN: z.number().optional().default(7 * 24 * 60 * 60),
});
...
export const config: { PORT: number; NODE_ENV: NodeEnv; JWT_SECRET: string, JWT_EXPIRES_IN: number, REFRESH_EXPIRES_IN: number } = parsed.data;
